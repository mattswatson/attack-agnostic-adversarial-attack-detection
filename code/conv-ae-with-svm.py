...
dataset = ShapDatasetFly(args.normal_path, args.adversarial_path, args.collate_path, args.model, normal_only=True,
                         large_normal=True)
dataset_all = ShapDatasetFly(args.normal_path, args.adversarial_path, args.collate_path, args.model, large_normal=True)
...
with tqdm.tqdm(total=len(dataloader)) as progress:
    for inputs, labels in dataloader:
        ...
        error = torch.sum((inputs[0] - output[0]) * (inputs[0] - output[0]))
