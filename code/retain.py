...
class RETAIN(nn.Module):
    def __init__(self, dim_input, dim_emb=128, dropout_emb=0.6, dim_alpha=128, dim_beta=128, dropout_context=0.6,
                 dim_output=2, batch_first=True):
    ...
    def forward(self, x, lengths, single_output=False):
        if self.batch_first:
...
def retain_epoch(loader, model, criterion, optimizer=None, train=False, verbose=False):
    ...
    for bi, batch in enumerate(tqdm(loader, desc="{} batches".format(mode), leave=False)):
        ...
        if verbose:
            ...
            print(output)
            print(target_var)
            print('predicted class:', F.softmax(output))
