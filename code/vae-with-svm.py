...
# Get test and train sets with even class distribution
sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2)
split = sss.split(dataset.df, dataset.labels)

train_indices, val_indices = next(split)

# We want to train on only the normal SHAP values
train_indices_normal = []
for index in train_indices:
    if dataset[index][1] == 0:
        train_indices_normal.append(index)

train_sampler = torch.utils.data.sampler.SubsetRandomSampler(train_indices)
train_sampler_normal = torch.utils.data.sampler.SubsetRandomSampler(train_indices_normal)
...
train_loader_normal = DataLoader(dataset, batch_size=batch_size, sampler=train_sampler_normal)
...
if args.load is None:
    ...
    for epoch in range(epochs):
        for data, _ in train_loader_normal:
